          ...
          <div>
            ...
            {detailsOpen && (
              <div style={{ marginTop: 8, display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'center' }}>
                ...
                <span style={{ fontSize: 12, color: '#555' }}>
                  Tipo: <strong>{isAdmin ? 'ADMIN' : isStaff ? 'STAFF' : isPartner ? 'PROVEEDOR' : 'CLIENTE'}</strong>
                </span>
                <span style={{ fontSize: 12, color: '#555' }}>
                  PhoneLine: <strong>{conversation?.phoneLine?.alias || conversation?.phoneLineId || '—'}</strong>
                </span>
                {canAssignConversation && isClientConversation ? (
                  <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, color: '#555' }}>
                    Asignado a:
                    <select
                      value={assignedToId}
            ...
            {detailsOpen && isClientConversation && namePanelOpen && (
            ...
            {detailsOpen && isClientConversation ? (
              noContact ? (
